  ...
  */
    ...
    public:
        ...
        void DrawEarth()
        {
            ...
            //m_drawMeshProgram->setMat4("MVP",  m_projView * m_modelEarth);
            //m_drawMeshProgram->setMat4("MVP", m_proj * m_view * m_modelEarth);
        }
        ...
        void draw(float delta, Shader shader)
        {
            ...
            //shader.use();
            //shader.setMat4("M", m_modelAtmos);
            ////m_atmosphereProgram->set_mat4("MVP", m_projView * m_modelAtmos);
            //shader.setMat4("MVP", m_proj * m_view * m_modelAtmos);

            //shader.setVec3("viewPos", m_viewPos);
            shader.setInt("viewSamples", viewSamples);
            shader.setInt("lightSamples", lightSamples);

            shader.setFloat("I_sun", I_sun);
            shader.setFloat("R_e", R_e);
            shader.setFloat("R_a", R_a);
            shader.setVec3("beta_R", beta_R);
            shader.setFloat("beta_M", beta_M);
            shader.setFloat("H_R", H_R);
            shader.setFloat("H_M", H_M);
            shader.setFloat("g", g);

            // 3. Handle GUI stuff
            //shader.setFloat("toneMappingFactor", m_toneMapping * 1.0);
            //if (m_animateSun)
            //{
    ...
    private:
        ...
        inline static const float defSunAngle = glm::radians(47.f);
